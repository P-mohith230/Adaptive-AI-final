...
import { Upload, FileText, X, CheckCircle2, AlertCircle, Settings2, Download, Scale } from 'lucide-react'
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from '@/components/ui/dialog'
import { PageHeader } from '@/components/page-header'
import { AssetImportPanel } from '@/components/asset-import-panel'
import { ReconciliationBatchImportPanel } from '@/components/reconciliation-batch-import-panel'
...
export default function ImportPage() {
  ...
  const tab = rawTab === 'investments' ? 'investments' : rawTab === 'reconciliation' ? 'reconciliation' : 'transactions'

  function selectTab(next: 'transactions' | 'investments' | 'reconciliation') {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title={
          tab === 'reconciliation'
            ? '3-Way Reconciliation Batch (Track 04)'
            : tab === 'investments'
            ? t('assetImport.title')
            : t('import.subtitle')
        }
      ...
      {tab !== 'reconciliation' && (
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 rounded-lg border border-primary/30 bg-primary/5 p-3.5 text-xs">
          <div className="flex items-center gap-2.5">
            <div className="rounded-md bg-primary/10 p-1.5 text-primary">
              <Scale className="h-4 w-4" />
            </div>
            <div>
              <span className="font-semibold text-foreground">Razorpay AI Finance Controller:</span>{' '}
              <span className="text-muted-foreground">
                Need to ingest and verify a 50+ batch across Merchant Orders, Razorpay Captured Payments, and Bank Settlements?
              </span>
            </div>
          </div>
          <button
          ...
          >
            Open 3-Way Reconciliation Importer →
          </button>
        </div>
      )}

      <div className="inline-flex items-center rounded-lg border border-border bg-muted/40 p-0.5">
        {(['transactions', 'investments', 'reconciliation'] as const).map((value) => (
          <button
            key={value}
            type="button"
      ...
      {tab === 'reconciliation' ? (
        <ReconciliationBatchImportPanel />
      ) : tab === 'investments' ? (
        <AssetImportPanel />
      ) : (
