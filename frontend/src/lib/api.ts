...
export const rules = {
  ...
  exportFile: async (): Promise<void> => {
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = `adaptiveai-categorization-rules-${new Date().toISOString().slice(0, 10)}.json`
    document.body.appendChild(a)
    a.click()
    document.body.removeChild(a)
    URL.revokeObjectURL(url)
...
export const assets = {
  ...
  importTemplate: async (): Promise<void> => {
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = 'adaptiveai-asset-orders.csv'
    document.body.appendChild(a)
    a.click()
    document.body.removeChild(a)
    URL.revokeObjectURL(url)
...
export const backup = {
  ...
  download: async (password?: string): Promise<void> => {
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = `adaptiveai-backup-${new Date().toISOString().slice(0, 10)}.zip`
    document.body.appendChild(a)
    a.click()
    document.body.removeChild(a)
    URL.revokeObjectURL(url)
...
export const reconciliation = {
  getKPI: async () => {
    const { data } = await api.get('/reconciliation/kpi')
    return data
  },
  runDemo: async (totalRecords: number = 100) => {
    const { data } = await api.post(`/reconciliation/demo-run?total_records=${totalRecords}`)
    return data
  },
  getBatches: async (limit: number = 20) => {
    const { data } = await api.get(`/reconciliation/batches?limit=${limit}`)
    return data
  },
  getRecords: async (params?: { batch_id?: string; status?: string; limit?: number; offset?: number }) => {
    const { data } = await api.get('/reconciliation/records', { params })
    return data
  },
  getDecisionCard: async (recordId: string) => {
    const { data } = await api.get(`/reconciliation/records/${recordId}/decision-card`)
    return data
  },
  reviewRecord: async (recordId: string, action: 'APPROVE' | 'REJECT' | 'RESOLVE', notes?: string) => {
    const { data } = await api.post(`/reconciliation/records/${recordId}/review`, { action, notes })
    return data
  },
  getAuditLog: async (limit: number = 50) => {
    const { data } = await api.get(`/reconciliation/audit-log?limit=${limit}`)
    return data
  },
  getCashForecast: async () => {
    const { data } = await api.get('/reconciliation/cash-forecast')
    return data
  },
  getSampleCsvs: async (totalRecords: number = 100) => {
    const { data } = await api.get(`/reconciliation/sample-csvs?total_records=${totalRecords}`)
    return data
  },
  importCsvBatch: async (ordersCsv: string, paymentsCsv: string, bankCsv?: string) => {
    const { data } = await api.post('/reconciliation/import-csv-batch', {
  ...
  syncLedger: async (batchId?: string) => {
    const { data } = await api.post('/reconciliation/sync-ledger', null, {
      params: batchId ? { batch_id: batchId } : undefined,
    })
    return data
  },
  askSettlementQA: async (question: string, history?: Array<{ role: string; content: string }>) => {
    const { data } = await api.post('/reconciliation/settlement-qa', { question, history })
    return data
  },
}

export const merchantLedger = {
  ...
  syncRazorpay: async () => {
    const { data } = await api.post('/ledger/sync-razorpay')
    return data
  },
}
